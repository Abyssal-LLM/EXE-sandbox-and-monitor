...
from PySide6.QtCore import Qt, Signal, Slot, QTimer, QRegularExpression
from PySide6.QtGui import (
    QTextCharFormat, QColor, QTextCursor, QFont,
    QSyntaxHighlighter, QContextMenuEvent,
)
...
class TerminalWidget(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        # Whether Knowledge Base mode is active (click-to-explain)
    ...
    def contextMenuEvent(self, event) -> None:
        """Handle right-click context menu on the terminal."""
        menu = QMenu(self)
        ...
        cursor = self.terminal_output.cursorForPosition(event.pos())
        cursor.select(QTextCursor.LineUnderCursor)
        selected_line = cursor.selectedText()

        # Explain action
        explain_action = menu.addAction("Explain in Knowledge Base")
        explain_action.triggered.connect(lambda: self.explain_requested.emit(selected_line))
    ...
    def eventFilter(self, obj, event) -> bool:
        """Handle mouse clicks on the terminal output for click-to-explain."""
        if obj == self.terminal_output and self.kb_active:
            from PySide6.QtCore import QEvent
            if event.type() == QEvent.MouseButtonPress:
                if event.button() == Qt.LeftButton:
                    cursor = self.terminal_output.cursorForPosition(event.pos())
                    cursor.select(QTextCursor.LineUnderCursor)
                    clicked_line = cursor.selectedText()
                    if clicked_line.strip():
                        self.explain_requested.emit(clicked_line)
                    return True

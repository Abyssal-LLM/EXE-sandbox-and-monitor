...
class SandboxEngineScript:
    ...
    def get_stats(self) -> dict:
        ...
        if self.session_start_time > 0:
            if self.is_running:
                ...
                duration = self.process_manager.processes[
                    list(self.process_manager.processes.keys())[0]
                ].start_time - self.session_start_time if self.process_manager.processes else 0
    ...
    def get_process_tree(self) -> list:
        ...
        """
        return self.process_manager.get_process_tree()
    ...
    def __del__(self):
        """Destructor - ensure we clean up when the object is garbage collected."""
        self.cleanup()


class SandboxConfig:
    """
    Configuration for the sandbox behavior.

    This holds all the settings that control how the sandbox operates.
    The GUI can modify these settings to customize the sandbox behavior.
    """

    def __init__(self):
        # Memory limit in MB - this is the maximum total memory for all processes
        self.max_memory_mb: int = 2048

        # Maximum number of concurrent processes
        self.max_processes: int = 32

        # CPU usage cap (0-100) - processes can't use more than this
        self.max_cpu_percent: int = 80

        # Monitoring intervals in seconds
        self.process_monitor_interval: float = 0.5
        self.file_monitor_interval: float = 1.0
        self.registry_monitor_interval: float = 2.0
        self.network_monitor_interval: float = 1.0

        # Whether to enable each type of monitoring
        self.enable_process_monitoring: bool = True
        self.enable_file_monitoring: bool = True
        self.enable_registry_monitoring: bool = True
        self.enable_network_monitoring: bool = True

        # Logging settings
        self.max_log_lines: int = 10000
        self.log_timestamp_format: str = "%H:%M:%S"
